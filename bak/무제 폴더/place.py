...
class Place:
        ...
        if self.owner is None:
            ...
            if playerInfo[index].money >= self.fee * 4:
                ...
                if playerInfo[index].totalMoney < self.fee:
                    ...
                    while True:
                        self.selectSellBuilding(game, display, playerInfo, index)
        ...
        while not done:
            ...
            if yesButton.result:
                if self.owner is not None:
                    playerInfo[self.owner].sellBuilding(self.name, self.fee * 2)
                    self.owner = None
    ...
    def selectSellBuilding(self, game, display, playerInfo, index):
        ...
        while not done:
            for i, j in enumerate(playerInfo[index].building):
                if button.Button.placeButton(placeLocation[list_placeName.index(j)]):
                    playerInfo[index].sellBuilding(placeName[list_placeName.index(j)], placeFee[list_placeName.index(j)] * 2)
                    playerInfo[index].getMoney(placeFee[list_placeName.index(j)] * 2)
                    self.owner = None
                    done = True
                    break
