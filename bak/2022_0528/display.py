...
class Display:
    ...
    def playerInfo(self, game, playerInfo):
        ...
        for i in range(game.playerNum):
            playerRank.append(playerInfo[i].totalMoney)

        playerRank = set(playerRank)
        playerRank = list(playerRank)
        playerRank.sort(reverse=True)

        for i in playerRank:
            for j in range(game.playerNum):
                if playerInfo[j].totalMoney == i:
                    playerInfo[j].rank = playerRank.index(i)
    ...
    def text(self, text, location):
        font = pygame.font.SysFont('applegothic', 12)
