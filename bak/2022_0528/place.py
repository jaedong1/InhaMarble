    ...
    def normalPlace(self, game, display, playerInfo, index):
        if self.owner is None:
            if playerInfo[index].money >= self.fee * 2:
                ...
                while not done:
                    yesButton = button.Button(display, display.yesImage, display.yesImage2, display.buttonLocation[0], True)
                    noButton = button.Button(display, display.noImage, display.noImage2, display.buttonLocation[1], True)
                    game.exitCheck()

                    if yesButton.result:
                        playerInfo[index].buyBuilding(self)
                        done = True
